...
test("global catalog CLI creates, binds, and inspects a Stack from any directory", async () => {
  ...
  try {
    ...
    const artifactGuidance = context.artifactGuidance as Array<{ artifact: { name: string }; localFallback: { dependencySpecifier: string }; instruction: string }>;
    ...
    assert.match(artifactGuidance[0]?.instruction ?? "", /relationship records intent; it does not make .* importable/);
    const renamedCapability = runJson(["component", "rename-capability", "tests/global-cli", "knowledge", "practice.engineering", "--to", "practice.engineering.v2"], 0, env);
    assert.deepEqual(renamedCapability.updatedConsumers, ["app"]);
    runJson(["component", "rename-capability", "tests/global-cli", "knowledge", "practice.engineering.v2", "--to", "practice.engineering"], 0, env);
    assert.equal(object(runJson(["component", "consume", "tests/global-cli", "app", "practice.engineering", "--optional"], 0, env).component).consumes instanceof Array, true);
    const requiredAgain = runJson(["component", "consume", "tests/global-cli", "app", "practice.engineering", "--required", "--clear-from"], 0, env);
    assert.deepEqual((object(requiredAgain.component).consumes as Array<Record<string, unknown>>)[0], { capability: "practice.engineering", optional: false });
    runJson(["component", "consume", "tests/global-cli", "app", "practice.engineering", "--from", "knowledge"], 0, env);
    runJson(["component", "unguidance", "tests/global-cli", "app", "--path", "AGENTS.md"], 0, env);
    runJson(["component", "guidance", "tests/global-cli", "app", "--path", "AGENTS.md", "--strength", "preferred"], 0, env);
