...
test("proposes a first-class HTTP application contract without overstating current behavior", () => {
  ...
  assert.match(
    proposal,
    /current `\/api\/v0\.1` routes remain pre-stable implementation evidence/u,
  );
...
test("keeps exercised MVP acceptance separate from its archived design narrative", () => {
  ...
  assert.match(
    catalog,
    /"id": "mvp-agent-workflow-vision", "category": "archive", "lifecycle": "archive"/u,
  );
  ...
  assert.match(
    doc("08-roadmap.md"),
    /cross-component capability requests as a complete vertical slice/u,
  );
...
test("documents bounded provider descriptors and explicit Stack precedence", () => {
  ...
  assert.match(
    doc("adr/0013-provider-descriptors-are-bounded-inputs.md"),
    /Consumer requirements remain Stack-owned and explicit/u,
  );
...
test("documents canonical Markdown as the web documentation source", () => {
  ...
  const catalog = readFileSync(
    new URL("../apps/web/lib/documentation.ts", import.meta.url),
    "utf8",
  );
  ...
  for (const lifecycle of ["current", "proposed", "decision", "archive"])
    assert.match(lifecycleCatalog, new RegExp(`"lifecycle": "${lifecycle}"`, "u"));
  for (const category of [
    ...
    assert.match(catalog, new RegExp(category, "u"));
  for (const parameter of ["view", "document", "heading"])
    assert.match(page, new RegExp(`searchParams\\.(?:get|set)\\('${parameter}'`, "u"));
...
test("documents the unified Components workspace and supplementary Docker quality gate", () => {
  assert.match(
    doc("architecture.md"),
    /Graph renders the declarative provider\/dependency relationships/u,
  );
  assert.match(doc("architecture.md"), /deterministic top-down layered SVG/u);
  assert.match(doc("user-guide.md"), /List and Graph share one search, one selected component/u);
  assert.match(doc("user-guide.md"), /Overview, Capabilities, Relationships, and Guidance/u);
...
test("documents every CLI operation with focused reference sections", () => {
  ...
  const operations = [
    ...
    "component bind",
    "component provide",
    "component rename-capability",
    "component unprovide",
    "component consume",
    "component unconsume",
    "component guidance",
    "component unguidance",
    "locate",
    "agent print",
    "agent check",
    "agent install",
  ...
  for (const operation of operations)
    assert.match(reference, new RegExp("### `stacks " + operation + "`", "u"));
  assert.match(
    reference,
    /Purpose|Creates|Lists|Returns|Reports|Builds|Starts|Runs|Appends|Writes/u,
  );
...
test("publishes complete MCP instructions, tools, resources, and examples", () => {
  ...
  for (const uri of [
    ...
    assert.match(reference, new RegExp(uri.replaceAll("/", "\\/"), "u"));
