...
4. At the start of every participating agent turn, use turn_start with the sessionId and current task. Retain its turnId, use the returned bounded briefing before relying on cross-component knowledge or capabilities, and review every omission, truncation, and artifact-guidance entry. A capability relationship records intent; it does not install or link an artifact. When the task needs a provided package, inspect both package manifests, workspace configuration, and the lockfile. If the dependency is not already usable, edit the consumer package.json using the existing workspace or registry convention, or use the exact localFallback.packageJson entry when no convention applies. Then run the repository's normal install and verification workflow when authorized. Stacks supplies guidance but does not edit package metadata or run the package manager.
...
Component tools can inspect, add, bind, and update existing local directories; declare, edit, rename, and remove capability relationships; and configure or remove component-relative guidance. Use the CLI for Git cloning or synchronization. Read stacks://reference/mcp for the full MCP tool/resource reference and stacks://reference/cli for explicit CLI-only operations.`;
