...
4. At the start of every participating agent turn, use turn_start with the sessionId and current task. Retain its turnId, use the returned bounded briefing before relying on cross-component knowledge or capabilities, and review every omission, truncation, and artifact-guidance entry. Preserve existing project registry and workspace conventions. A derived local file dependency is fallback guidance only when no established strategy applies; Stacks does not run the package manager.
