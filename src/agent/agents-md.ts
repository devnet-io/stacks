...
const instructions = [
  ...
  "4. At the beginning of each participating turn, call `turn_start`. Retain the returned `turnId`, follow its bounded provenance-bearing briefing, review every omission or truncation, and inspect any artifact guidance. A Stack capability relationship does not install or link its artifact. When the task needs a provided package, inspect package manifests, workspace configuration, and the lockfile; if it is not already usable, edit the consumer package.json using the existing workspace or registry convention, or the exact `localFallback.packageJson` entry when no convention applies. Run the repository's normal install and verification workflow when authorized. Stacks does not edit package metadata or run the package manager. Close the turn with `turn_complete`, including only telemetry the client actually knows.",
