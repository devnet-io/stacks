...
- `stacks mcp` is one machine-catalog stdio adapter. It supplies standard initialization instructions, agent-readable instruction/CLI/MCP resources, and tools for discovery, component/context configuration, resolution, and lifecycle reporting. Stack-specific MCP tools require `stack`; there is no local MCP port. A canonical MCP catalog is checked against the source transport and the packed installed transport. Agent clients own their loaded tool registries, so registration and MCP-contract upgrades require a client restart.
