...
Registered Stack creation, component addition, changed component bindings, work starts, completed turns, work completion, and usage are append-only JSONL events under a registered Stack's state directory. Management events are appended by the shared application boundary after the catalog mutation commits; adapter provenance identifies CLI, web, or MCP, and unchanged bindings do not create duplicates. Historical operations are never backfilled from current state. Independent processes serialize appends through a per-Stack exclusive lock and sync each record before releasing it; readers never acquire the writer lock. A lock timeout fails with its exact path instead of deleting a possibly live lock. Monetary values require `reported`, `estimated`, or `allocated` provenance. Read commands never mutate component repositories. Git sync may clone to an explicit path or fetch with `--update`, but never resets, cleans, merges, rebases, overwrites dirty work, or silently moves a repository.
