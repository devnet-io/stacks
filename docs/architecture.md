...
Context resolution is deterministic and provenance-rich. Registered Stacks can upsert capability exports with one bounded context path, consumer requirements with an optional explicit provider, and component-relative guidance through the shared CLI, MCP, HTTP, and Manage adapters. It verifies file-backed paths against the owning component root and reports missing or ambiguous providers. Starting a participating agent turn resolves this plan for the session's target component and returns it with a durable `turnId`; task text remains transient rather than being copied into activity history. The durable turn-start event currently records plan counts and generation time, not a context revision, digest, selected item identities, or materialized content, so the exact returned plan cannot yet be reconstructed from activity history. Task-aware selection, content materialization, refresh cadences, explicit byte/token enforcement, removal and multi-path editing, and cross-component capability requests remain unfinished.
