...
Task terms rank declared path, description, tag, reason, and capability metadata within each guidance strength; declared priority and deterministic ties follow. Standalone context resolution defaults to a 32 KiB orientation. The first turn in a session receives the same orientation cadence, while subsequent turns use an 8 KiB refresh; callers may set a hard positive byte budget up to 256 KiB. The returned briefing has a stable digest, and `turn.started` records that digest plus mode/budget/count metadata without persisting task text or file contents. Refresh currently means a smaller current-plan materialization rather than a change-aware delta. Model-specific token enforcement, removal and multi-path editing, and cross-component capability requests remain unfinished.
