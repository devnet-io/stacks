...
The UI is machine-level. A subdued Stack selector sits directly below the sidebar header and applies to every operational section. Components is the primary composition workspace: List and Graph share one search, one selected component, and the same live details. Wide displays keep those details in a stable rail beside the explorer; smaller desktops use a compact selection bar so the graph retains useful width. Edit opens a full component workspace rather than an overlay. Its Overview, Capabilities, Relationships, and Guidance sections separate descriptive metadata and local bindings from provider, consumer, artifact, descriptor, and guidance operations. Add component likewise uses an in-page workflow. Activity separates logical work from Stack changes and links to work/turn details, Requests provides paginated request creation/list/detail/transition views, Tools & agents contains runtime connection instructions, and Documentation renders every canonical Markdown file under `docs/`. The bottom application menu displays the installed version and reserves space for future account/settings controls. Installation instructions live in Documentation rather than the operational tabs.
...
Capability consumption is required unless explicitly marked optional. A missing or ambiguous required provider is a context error; an optional one remains visible but is a warning. The Relationships editor labels every saved requirement as required or optional. Graph lays providers and dependencies above their consumers and distinguishes capability, dependency, and optional edges. Selecting a node updates the same component selection used by List. Direct component dependencies are currently always required.
...
The Capabilities editor shows whether the descriptor is absent, valid, invalid, or unavailable, along with published, applied, and overridden capabilities. Invalid descriptors are ignored without changing the Stack definition. Edit and review the descriptor in the component repository through its normal Git workflow; Stacks does not create or update it.
