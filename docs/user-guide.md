...
The UI is machine-level. A subdued Stack selector sits directly below the sidebar header and applies to every operational section. Components is the primary composition workspace: its List presentation shows live health and repository state, while Graph shows provider and dependency relationships. Selecting a row or graph node opens a right-side details inspector without moving the selection below the fold. The inspector edits component display metadata, local bindings, Stack-owned providers and consumers, artifacts, and guidance, and displays provider-descriptor provenance and validation failures. The nearby Add action attaches components or creates another Stack. Activity separates logical work from Stack changes and links to work/turn details, Requests provides paginated request creation/list/detail/transition views, Tools & agents contains runtime connection instructions, and Documentation renders every canonical Markdown file under `docs/`. The bottom application menu displays the installed version and reserves space for future account/settings controls. Installation instructions live in Documentation rather than the operational tabs.
...
Capability consumption is required unless explicitly marked optional. A missing or ambiguous required provider is a context error; an optional one remains visible but is a warning. The component inspector labels every saved requirement as required or optional. Graph lays providers and dependencies above their consumers and distinguishes capability, dependency, and optional edges in a compact legend. Selecting a node opens the same inspector used by List. Direct component dependencies are currently always required.
...
The component inspector shows whether the descriptor is absent, valid, invalid, or unavailable, along with published, applied, and overridden capabilities. Invalid descriptors are ignored without changing the Stack definition. Edit and review the descriptor in the component repository through its normal Git workflow; Stacks does not create or update it.
