...
`stacks ui` presents the machine catalog with a Stack selector above its section navigation. Components, Activity, Requests, Tools & agents, and canonical documentation use the same application semantics available to agents. Components switches between list and graph presentations of one composition surface. Selecting a row or graph node opens a side inspector that combines component evidence with display metadata, local binding, provider, consumer, artifact, and guidance editing; existing-provider and existing-requirement selectors make upsert behavior explicit. The graph labels required and optional relationships in both resolved and unresolved states. Requests exposes the cross-component request protocol without becoming a scheduler. A bottom application menu shows the installed version; Tools & agents contains runtime connection settings, while installation remains in canonical documentation.
