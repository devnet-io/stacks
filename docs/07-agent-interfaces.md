...
`stacks ui` presents the machine catalog with a Stack selector above its section navigation. Components, Activity, Requests, Tools & agents, and canonical documentation use the same application semantics available to agents. Components is one explorer whose list and graph presentations share search, selection, and a live component summary. Wide screens reserve stable space for full selection details; smaller desktop screens keep a compact selection bar above the full-width canvas. Editing leaves exploration mode for a dedicated component workspace with Overview, Capabilities, Relationships, and Guidance sections; existing-provider and existing-requirement selectors make upsert behavior explicit. The graph labels required and optional relationships in both resolved and unresolved states. Requests exposes the cross-component request protocol without becoming a scheduler. A bottom application menu shows the installed version; Tools & agents contains runtime connection settings, while installation remains in canonical documentation.
