...
| Global stdio MCP | One `stacks mcp` registration discovers direct or ancestor workspace membership; lists, inspects, adds, and binds local components; configures capability providers/requirements and guidance; safely materializes context; and exposes the work/turn lifecycle. `turn_start` returns a bounded briefing plus top-level session/turn identities, `turn_complete` accepts known live telemetry, and `usage_import` is reserved for delayed measurements. Every selected-Stack tool requires a selector, Git cloning stays CLI-only, and source plus packed-install transport checks compare actual tools/resources with the canonical catalog. `stacks doctor` reports the installed contract and the agent-client restart boundary. |
