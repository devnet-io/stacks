...
| Global stdio MCP | One `stacks mcp` registration discovers workspace memberships and can list, inspect, add, and bind local components. Every selected-Stack tool requires a selector, Git cloning stays CLI-only, and source plus packed-install transport checks compare actual tools/resources with the canonical catalog. `stacks doctor` reports the installed contract and the agent-client restart boundary. |
