...
| Local documentation UI | `apps/web` auto-discovers all `docs/**/*.md` sources, groups them by truth type, nests selected-document headings in the same sidebar, preserves repository-relative document links, and supports stable URL deep links. |
...
| Global stdio MCP | One `stacks mcp` registration discovers workspace memberships and can list, inspect, add, and bind local components. Every selected-Stack tool requires a selector, Git cloning stays CLI-only, and the transport test protects stdout. |
