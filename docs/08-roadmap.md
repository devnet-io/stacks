...
3. Complete write-operation parity for Stack/component management through the application boundary, versioned HTTP API, CLI, MCP, and full UI editing slices. Creation, membership discovery, local add/bind, stable-ID component metadata editing, and context-graph authoring with explicit required/optional relationships are complete; removal and richer multi-path editing remain.
