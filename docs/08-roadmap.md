...
3. **Complete for the MVP graph:** write-operation parity for Stack/component management through the application boundary, versioned HTTP API, CLI, MCP, and full UI editing slices. Creation, membership discovery, local add/bind, stable-ID metadata editing, exact declaration updates/removals, atomic capability rename, and explicit required/optional relationships are implemented. Richer multi-path authoring remains a later context refinement rather than an MVP editing gap.
