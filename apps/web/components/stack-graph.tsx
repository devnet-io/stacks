...
function ComponentDetails({ node, edges, nodes, onSelect }: { node: StackGraphNode; edges: StackGraphEdge[]; nodes: StackGraphNode[]; onSelect(id: string): void }) {
  ...
  return <Card className="xl:sticky xl:top-28"><CardHeader><div className="flex items-center gap-2"><Badge variant="secondary">{node.kind}</Badge><Badge variant="outline">{node.sourceType}</Badge></div><CardTitle className="mt-2 text-lg">{node.name}</CardTitle><CardDescription className="font-mono text-xs">{node.id}</CardDescription></CardHeader><CardContent className="space-y-5">{node.description && <p className="text-sm leading-6 text-muted-foreground">{node.description}</p>}<CapabilityList icon={<Boxes />} label="Provides" values={node.provides} empty="No capabilities exported" /><CapabilityList icon={<Network />} label="Consumes" values={node.consumes} empty="No capabilities required" /><Relations label="Receives from" edges={incoming} other={(edge) => byId.get(edge.from)} onSelect={onSelect} /><Relations label="Feeds into" edges={outgoing} other={(edge) => byId.get(edge.to)} onSelect={onSelect} /></CardContent></Card>;
}
