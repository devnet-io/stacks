...
export function StackGraph({ stack }: { stack?: string }) {
  ...
  return <div className="space-y-6">
    ...
    {data.unresolved.length > 0 && <Alert><TriangleAlert /><AlertTitle>{data.unresolved.length} unresolved capability requirement{data.unresolved.length === 1 ? '' : 's'}</AlertTitle><AlertDescription>{data.unresolved.map((item) => `${item.componentId}: ${item.capability} (${item.reason})`).join(' · ')}</AlertDescription></Alert>}
...
function ComponentDetails({ node, edges, nodes, onSelect }: { node: StackGraphNode; edges: StackGraphEdge[]; nodes: StackGraphNode[]; onSelect(id: string): void }) {
  ...
  return <Card className="xl:sticky xl:top-28"><CardHeader><div className="flex items-center gap-2"><Badge variant="secondary">{node.kind}</Badge><Badge variant="outline">{node.sourceType}</Badge></div><CardTitle className="mt-2 text-lg">{node.name}</CardTitle><CardDescription className="font-mono text-xs">{node.id}</CardDescription></CardHeader><CardContent className="space-y-5">{node.description && <p className="text-sm leading-6 text-muted-foreground">{node.description}</p>}<CapabilityList icon={<Boxes />} label="Provides" values={node.provides} empty="No capabilities exported" /><CapabilityList icon={<Boxes />} label="Artifacts" values={node.artifacts.map((artifact) => `${artifact.ecosystem}:${artifact.name} · ${artifact.capability}`)} empty="No implementation artifacts declared" /><CapabilityList icon={<Network />} label="Consumes" values={node.consumes} empty="No capabilities required" /><Relations label="Receives from" edges={incoming} other={(edge) => byId.get(edge.from)} onSelect={onSelect} /><Relations label="Feeds into" edges={outgoing} other={(edge) => byId.get(edge.to)} onSelect={onSelect} /></CardContent></Card>;
}
function Relations({ label, edges, other, onSelect }: { label: string; edges: StackGraphEdge[]; other(edge: StackGraphEdge): StackGraphNode | undefined; onSelect(id: string): void }) { return <div><p className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[.12em] text-muted-foreground"><GitBranch className="size-3.5" />{label}</p>{edges.length ? <div className="space-y-1">{edges.map((edge) => { const node = other(edge); return <button type="button" key={edge.id} onClick={() => node && onSelect(node.id)} className="flex w-full items-center gap-2 rounded-lg p-2 text-left hover:bg-muted"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{node?.name ?? 'Unknown'}</span><span className="block truncate text-xs text-muted-foreground">{edge.label}</span></span><ArrowRight className="size-3.5 text-muted-foreground" /></button>; })}</div> : <p className="text-xs text-muted-foreground">No relationships</p>}</div>; }
