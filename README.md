...
`stacks ui` opens one local management interface for the machine catalog. Use the Stack selector to switch between registered Stacks; Activity shows Stack changes, agent work, and usage, while Manage can create Stacks, add components, edit display metadata, change local bindings, and configure artifacts and required/optional capability relationships. The UI includes the same getting-started and reference documentation as this repository.
...
stacks component update <namespace/name> <id> [--name <name>] [--description <text>] [--kind <kind>] [--access read-only|read-write] [--json]
...
`npm run install:local` is idempotent: it builds a complete package and installs that snapshot into npm's machine-level package storage. The installed CLI and built web UI do not link back to this checkout, so the clone may be moved or deleted afterward. Rerun the command when you want to install newer source changes. `npm run docs:check` audits documentation lifecycle coverage and relative links. `npm run check` includes that audit, creates a clean temporary installation, verifies its complete MCP contract and three-component workflow, then starts its web runtime. `npm run check:docker` repeats the same checks in clean Node 22 Linux userspace.
